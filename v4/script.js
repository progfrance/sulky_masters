...
function applyCardEffect(card, horse, dieValue) {
    ...
    const opponentName = state.players[opponentId].name;
    ...
    switch(card.id) {
        ...
        case 'ASPIRATION':
            ...
            if(found) {
                ...
                log(`💨 ${horse.name} aspiré case ${horse.pos} !`);
            } else {
        ...
        case 'CRAVACHE':
            // Avance immédiate de 3 cases
            horse.pos += 3;
            log(`⚡ Coup de Cravache ! +3 cases.`);
            checkFinishLine(horse);
            break;

        case 'DEFERRE':
            // Si le dé joué était 1, 2 ou 3, il devient 6.
            // Le cheval avance de (6 - dé joué).
            if(dieValue <= 3) {
                const bonus = 6 - dieValue;
                horse.pos += bonus;
                log(`⚡ Déferré ! Le ${dieValue} devient 6. Avance de ${bonus} cases.`);
                checkFinishLine(horse);
            } else {
                log("Déferré sans effet (dé > 3).");
            }
            break;

        case 'ENFERME':
            state.playerMalus[opponentId] = true;
            log(`🔒 ${opponentName} sera gêné au prochain tour !`);
            break;
    }
    renderHorses();
}
...
function updateMalusUI() {
    if(state.playerMalus[state.turn]) elMalusArea.style.display = 'block';
    else elMalusArea.style.display = 'none';
}
...
function calculateMoves() {
    ...
    const hasMalusEnferme = state.playerMalus[state.turn];
    ...
    state.dice.forEach((val, dIdx) => {
        ...
        p.horses.forEach(h => {
            ...
            // Règle ENFERMÉ
            if(hasMalusEnferme && h.status === 'racing') {
    ...
    if(moves===0) {
        ...
        if(hasMalusEnferme) msg += " (Malus Enfermé actif)";
        elMsg.innerText = msg;
...
function addChoiceBtn(h, val, dIdx, type, target, color, penalty) {
    ...
    let actionHtml = state.actionSquares.includes(target) ? ` <span style='color:violet; font-weight:bold'>[CARTE!]</span>` : '';

    btn.innerHTML = `<span class="die-icon">${val}</span> <b>${h.name}</b> : ${txt} ${penaltyHtml} ${actionHtml}`;
    // IMPORTANT : On passe 'val' (la valeur du dé) à executeMove
...
function executeMove(h, target, dIdx, type, dieValue) {
    ...
    // Carte Action ?
    // On passe le 'dieValue' pour le calcul du bonus Déferré
...
function endTurn() {
    ...
    state.playerMalus[state.turn] = false;
