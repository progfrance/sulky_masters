...
const state = {
    ...
    turn: 1,
    players: {
        1: { name: 'Rouges', color: 'p1', points: 0, horses: [] },
        2: { name: 'Bleus', color: 'p2', points: 0, horses: [] }
    }
};
...
function renderHorses() {
    ...
    elGlobalScoreP1.innerText = state.players[1].points;
    elGlobalScoreP2.innerText = state.players[2].points;
    ...
    [1,2].forEach(pid => {
        ...
        else stableEl.innerHTML = `<span id="label-p${pid}" class="stable-label ${pid===1?'p1':'p2'}-color">${state.players[pid].name}</span>`;
    ...
    [1,2].forEach(pid => {
        state.players[pid].horses.forEach(h => {
            if(h.status === 'racing') {
                ...
                // MODIFICATION : Suppression de h.icon
...
function showEnfermeTargetSelection() {
    ...
    const opponentHorses = state.players[opponentId].horses.filter(h => h.status === 'racing');
...
function applyEnferme(targetId) {
    ...
    let name = "Cible";
    [1,2].forEach(p => {
        const h = state.players[p].horses.find(x => x.id === targetId);
        if(h) name = h.name;
    });
...
elRoll.onclick = () => {
    ...
    const pName = state.players[state.turn].name;
    log(`${pName} : [${state.dice[0]}] [${state.dice[1]}]`);
...
function addChoiceBtn(h, val, dIdx, type, target, color, penalty) {
    ...
    // MODIFICATION : Suppression de h.icon, ajout de <b> pour le nom
...
function updateTurnIndicator() {
    ...
    elTurnInd.className = state.turn===1 ? 'bg-p1' : 'bg-p2';
}
