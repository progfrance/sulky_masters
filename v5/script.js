...
function initCoords() {
    for(let i=0; i<TRACK_LEN; i++) {
        ...
        COORDS.push({
            x: CX + RX * Math.cos(angle) - 25, // Coin haut-gauche de la case (50px)
...
const CARD_DEFINITIONS = [
    ...
    { id: 'ENFERME', name: "Enfermé", count: 8, desc: "L'adversaire ne pourra pas bouger un cheval bloqué au PROCHAIN tour." },
...
const state = {
    ...
    lockedHorseId: null, 
    players: {
        1: { name: 'Rouges', color: 'p1', horses: [] },
        2: { name: 'Bleus', color: 'p2', horses: [] }
    }
};
...
const elBtnCloseRules = document.getElementById('btn-close-rules');

elBtnCloseCard.onclick = () => { elModal.classList.add('hidden'); };
elBtnRules.onclick = () => { elRulesModal.classList.remove('hidden'); };
elBtnCloseRules.onclick = () => { elRulesModal.classList.add('hidden'); };

elBtnStart.onclick = startGame;

// --- INITIALISATION ---
function startGame() {
    const n1 = elInputP1.value.trim() || "Rouges";
    const n2 = elInputP2.value.trim() || "Bleus";

    state.players[1].name = n1;
    state.players[2].name = n2;

    elLabelP1.innerText = "Écurie " + n1;
    elLabelP2.innerText = "Écurie " + n2;
    elScoreNameP1.innerText = n1;
    elScoreNameP2.innerText = n2;
    elTurnInd.innerText = "TOUR DE " + n1.toUpperCase();

    state.players[1].horses = createTeam(n1, 'p1');
    state.players[2].horses = createTeam(n2, 'p2');
...
function createTeam(name, col) {
    ...
    return [1,2,3].map(i => ({ id: `${col}h${i}`, name: `${prefix}-${i}`, pos: -1, status: 'stable' }));
}
    ...
    [1,2].forEach(pid => {
        state.players[pid].horses.forEach(h => {
            ...
            if(h.status === 'stable') {
                ...
                // CALCULE DU CENTRAGE PRECIS (Case 50px, Cheval 34x22)
                const baseX = c.x + 25 - 17; // 25=demi-case, 17=demi-cheval
                const baseY = c.y + 25 - 11; // 11=demi-hauteur-cheval
                ...
                if (stack && stack.length > 1) {
                    if (stack.length === 2) {
                        offsetX = indexInStack === 0 ? -12 : 12; // Ecartement
                        offsetY = indexInStack === 0 ? -8 : 8;
                    } else {
                        offsetX = (indexInStack * 8) - 8;
                    }
                }
...
function updateDiceUI(d1, d2, onlyStyle) {
    ...
    [0,1].forEach(i => {
        ...
        // On garde toujours la classe de base .die (qui est jaune)
        // On ajoute juste 'used' si besoin
        if(state.diceUsed[i]) {
...
// ... (Le reste du code JS : isSquareOccupied, checkOvertake, drawCard, etc. reste inchangé) ...
// Pour être complet, je remets les fonctions inchangées ci-dessous :
...
function applyCardEffect(card, horse, dieValue) {
    ...
    switch(card.id) {
        ...
        case 'ASPIRATION':
            ...
            let found = false;
            [1,2].forEach(p => {
                state.players[p].horses.forEach(h => {
        ...
        case 'ENFERME':
            let opponentBestHorse = null;
            let maxPos = -999;
            state.players[opponentId].horses.forEach(h => {
            ...
            if (opponentBestHorse) {
                state.lockedHorseId = opponentBestHorse.id;
                log(`🔒 ${opponentBestHorse.name} sera bloqué au prochain tour s'il n'est pas seul !`);
            } else {
...
function updateMalusUI() {
    if(state.lockedHorseId) {
        ...
        if(isMyHorse) {
            ...
            elMalusText.innerText = `⚠️ ${lockedHorseName} est ENFERMÉ !`;
        } else {
...
function endTurn() {
    ...
    ui.innerText = `TOUR DE ${nextName.toUpperCase()}`;
